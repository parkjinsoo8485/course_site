...
const FAQ_CATEGORIES = [
  ...
  { category: '신청자 관리', items: [
    { title: '수강신청 테스트하고 싶어요', doc: '/help/go_data/num/76/data/link2', video: '/help/go_data/num/96/data/link1' },
...
// 운영 절차 HTML
let opsHtml = '';
for (const item of FAQ_PROCEDURES) {
  let badges = '';
  if (item.doc) badges += docBadge(item.doc, '문서') + ' ';
  if (item.video) badges += videoBadge(item.video);
  opsHtml += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 2px;border-bottom:1px solid #f3f4f6;font-size:0.83rem;">`;
  opsHtml += `<div style="display:flex;align-items:center;gap:6px;"><span style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;background:#2563eb;color:#fff;border-radius:50%;font-size:0.68rem;font-weight:700;flex-shrink:0;">${item.num}</span>`;
  opsHtml += `<span style="color:#1e293b;">${item.title}</span></div>`;
  opsHtml += `<div style="display:flex;gap:3px;flex-shrink:0;">${badges}</div></div>`;
}

// 양식 다운로드 HTML
let tmplHtml = '';
for (const item of FAQ_TEMPLATES) {
  const badges = item.links.map(lk => lk.type === 'video' ? videoBadge(lk.href) : docBadge(lk.href, lk.label)).join(' ');
  tmplHtml += `<div style="padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:0.85rem;"><span style="font-weight:600;color:#1e293b;">${item.title}</span> ${badges}</div>`;
}

// 매뉴얼 다운로드 HTML
let manHtml = '';
for (const item of FAQ_MANUALS) {
  const badges = item.links.map(lk => lk.type === 'video' ? videoBadge(lk.href) : docBadge(lk.href, lk.label)).join(' ');
  const color = item.isHighlight ? '#dc2626' : '#1e293b';
  manHtml += `<div style="padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:0.85rem;"><span style="font-weight:600;color:${color};">${item.title}</span> ${badges}</div>`;
}
...
// 기존 placeholder를 실제 콘텐츠로 교체
const ops_placeholder = '<div id="operationsListContainer" style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px;"><div style="color:#94a3b8;font-size:0.82rem;padding:8px 0;">로딩 중...</div></div>';
const ops_replacement = `<div id="operationsListContainer" style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px;">${opsHtml}</div>`;

const tmpl_placeholder = '<div id="templateDownloadsContainer"><div style="color:#94a3b8;font-size:0.82rem;padding:8px 0;">로딩 중...</div></div>';
const tmpl_replacement = `<div id="templateDownloadsContainer">${tmplHtml}</div>`;

const man_placeholder = '<div id="manualDownloadsContainer"><div style="color:#94a3b8;font-size:0.82rem;padding:8px 0;">로딩 중...</div></div>';
const man_replacement = `<div id="manualDownloadsContainer">${manHtml}</div>`;

const faqLeft_placeholder = '<div id="faqColLeft" style="display:flex;flex-direction:column;gap:10px;"><div style="color:#94a3b8;font-size:0.82rem;padding:8px 0;">로딩 중...</div></div>';
const faqLeft_replacement = `<div id="faqColLeft" style="display:flex;flex-direction:column;gap:10px;">${leftHtml}</div>`;

const faqRight_placeholder = '<div id="faqColRight" style="display:flex;flex-direction:column;gap:10px;"></div>';
const faqRight_replacement = `<div id="faqColRight" style="display:flex;flex-direction:column;gap:10px;">${rightHtml}</div>`;

let changed = 0;
if (content.includes(ops_placeholder)) { content = content.replace(ops_placeholder, ops_replacement); changed++; }
if (content.includes(tmpl_placeholder)) { content = content.replace(tmpl_placeholder, tmpl_replacement); changed++; }
if (content.includes(man_placeholder)) { content = content.replace(man_placeholder, man_replacement); changed++; }
if (content.includes(faqLeft_placeholder)) { content = content.replace(faqLeft_placeholder, faqLeft_replacement); changed++; }
if (content.includes(faqRight_placeholder)) { content = content.replace(faqRight_placeholder, faqRight_replacement); changed++; }

fs.writeFileSync(indexPath, content, 'utf8');
console.log(`Done. ${changed}/5 placeholders replaced with static HTML content.`);
